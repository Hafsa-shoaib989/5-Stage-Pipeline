...
reg_rd := io.EXMEM_rd
reg_dataMem_data := io.in_dataMem_out
...
io.MEMWB_dataMem_out := reg_dataMem_data
io.MEMWB_alu_out := reg_alu_out

dontTouch(io.MEMWB_dataMem_out)
dontTouch(io.MEMWB_alu_out)
dontTouch(reg_dataMem_data)
dontTouch(reg_alu_out)
}
